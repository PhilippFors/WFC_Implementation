...
class Training : MonoBehaviour
{
    ...
    public static byte Get2DByte(byte[,] ar, int x, int z)
    {
        return ar[x, z];
    }
    ...
    public void RecordNeighbors()
    {
        ...
        for (int z = 0; z < depth; z++)
        {
            for (int x = 0; x < width; x++)
            {
                for (int r = 0; r < 2; r++)
                {
                    int idx = (int)sample[x, z];
                    int rot = Card(RS[idx] + r);
                    int rx = x + 1 - r;
                    int rz = z + r;
                    if (rx < width && rz < depth)
                    {
                        int ridx = (int)sample[rx, rz];
                        ...
                        if (!neighbors.ContainsKey(key) && tiles[idx] && tiles[ridx])
                        {
                            ...
                            Debug.DrawLine(
                                transform.TransformPoint(new Vector3((x + 0f) * gridsize, 1f, (z + 0f) * gridsize)),
                                transform.TransformPoint(new Vector3((x + 1f - r) * gridsize, 1f, (z + 0f + r) * gridsize)), Color.red, 9.0f, false);
                        }
                    }
                }
            }
    ...
    public void Compile()
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
                  ...
                  (tilepos.z > -0.55f) && (tilepos.z <= depth * gridsize - 0.55f))
            {
                ...
                tile.name = fab.name;
#endif
                int X =  Mathf.RoundToInt((tilepos.x) / gridsize);
                int Z = Mathf.RoundToInt((tilepos.z) / gridsize);
                ...
                if (!str_tile.ContainsKey(fab.name + R))
                {
                    ...
                    tiles[index] = fab;
                    RS[index] = R;
                    sample[X, Z] = str_tile[fab.name + R];
                }
                else
                {
                    sample[X, Z] = str_tile[fab.name + R];
